...
import { getIn, setIn, shallowEqual } from '../utils'
...
export function createLens(set: any, get: any, path: any) {
  const normPath = path === undefined ? undefined : typeof path === 'string' ? [path] : path

  const _set = (partial: any, replace: any, ...args: any[]) =>
    set(
      (parentValue: any) => {
        const ourOldValue: any = normPath ? getIn(parentValue, normPath) : parentValue
        const ourTmpValue = typeof partial === 'function' ? partial(ourOldValue) : partial
        ...
        if (isDraft) {
          ...
          const pp = /*draft*/ ourOldValue2[meta]?.postprocess?.(draft, ourOldValue2, ...args)
          if (pp) Object.assign(draft, pp)
          return
        }

        const ourTmpValue2 = replace || !isPlain ? ourTmpValue : { ...ourOldValue, ...ourTmpValue }

        const ourNextValue = isPlain
          ? {
              ...ourTmpValue2,
              ...ourTmpValue2[meta]?.postprocess?.(ourTmpValue2, ourOldValue, ...args),
            }
          : ourTmpValue2

        const isSame = isPlain ? shallowEqual(ourOldValue as any, ourNextValue) : Object.is(ourOldValue, ourNextValue)
...
const isLens = (x: any): x is Lens<any> => !!x && x[LENS_TAG]

// https://stackoverflow.com/a/55541672
type IsAny<T> = 0 extends 1 & T ? true : false

export type ResolveStoreApi<X> =
  IsAny<X> extends true ? StoreApi<any> : X extends StoreApi<any> ? X : X extends unknown ? StoreApi<X> : unknown
...
export type Lens<T, S = unknown, SetterFn = Setter<T>, Ctx = LensContext<T, S>> = (
  set: SetterFn,
  get: Getter<T>,
  api: ResolveStoreApi<S>,
  ctx: Ctx,
...
export function lens<T, S = unknown>(fn: Lens<T, S, Setter<T>>): LensOpaqueType<T, S> {
  const self = (set: any, get: any, api: any, ctx: any /* partial context */) => {
...
const findLensAndCreate = (x: any, parentCtx: LensContext<any, any>) => {
  let res = x
  ...
  if (isPlainObject(x)) {
    ...
    keys.forEach(k => {
      ...
      if (isLens(v)) {
        ...
        let setterFn: any = (x: any) => x()

        const set = (...args: any[]) =>
          parentCtx.atomic(() => setterFn(() => (parentCtx.set as any)(...args), lensCtx))

        v = v(set, parentCtx.get, parentCtx.api, lensCtx)
        if ((v as any)[meta]?.setter) setterFn = (v as any)[meta].setter
        nextSet = lensCtx.set
        nextGet = lensCtx.get
        nextRelativePath = []
      }
...
type WithLensesImpl = <T>(f: StateCreator<T, [], []> | T) => StateCreator<T, [], []>

const withLensesImpl: WithLensesImpl = config => (set, get, api) => {
  const atomic = (api as any)[storeContext]?.atomic ?? atomicStub

  let setterFn: any = (x: any) => x()

  const setFn = (...args: any[]) => atomic(() => setterFn(() => (set as any)(...args), ctx))
...
>(
  f:
    ...
    | StateCreator<T, Mps, Mcs, CheckLenses<T & LensMeta<T, unknown>, Mutate<StoreApi<T>, Mps>>>,
...
const atomicStub = (fn: any) => fn()

const atomicStubWithWarning = (fn: any) => {
